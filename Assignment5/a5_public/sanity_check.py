...
def question_1g_sanity_check():
    ...
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1f: CNN CLASS ")
    print ("-"*80)
    out_channels = EMBED_SIZE
    kernel_size= 5
    max_len_word =10 
    inpt = torch.zeros( (BATCH_SIZE,EMBED_SIZE,max_len_word))
    CNN_source = cnn.CNN(EMBED_SIZE,out_channels,kernel_size,padding=1)
    output = CNN_source.forward(inpt)
    output_expected_size = [BATCH_SIZE, out_channels]
    assert(list(output.size()) == output_expected_size), "output shape is incorrect: it should be:\n {} but is:\n{}".format(output_expected_size, list(output.size()))
    print("Sanity Check Passed for Question 1f: CNN  CLASS!")
    print("-"*80)
