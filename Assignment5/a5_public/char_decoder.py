...
class CharDecoder(nn.Module):
    def __init__(self, hidden_size, char_embedding_size=50, target_vocab=None):
        ...
        self.vocab_size = len( self.target_vocab.char2id )
    ...
    def forward(self, input, dec_hidden=None):
        ...
        input = self.decoderCharEmb(input)  #(length,batch_size,char_embedding_size)
        output, dec_hidden = self.charDecoder( input, dec_hidden) 
        scores= self.char_output_projection(output)  #(length, batch_size, self.vocab_size)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        ###
        ### Hint: - Make sure padding characters do not contribute to the cross-entropy loss. Check vocab.py to find the padding token's index. : self.char_pad
        ###       - char_sequence corresponds to the sequence x_1 ... x_{n+1} (e.g., <START>,m,u,s,i,c,<END>). Read the handout about how to construct input and target sequence of CharDecoderLSTM.
        ###       - Carefully read the documentation for nn.CrossEntropyLoss and our handout to see what this criterion have already included:
        ###             https://pytorch.org/docs/stable/nn.html#crossentropyloss
        input = char_sequence[:-1,:]
        scores,_ =  self.forward(input,dec_hidden)  #(length, batch_size, self.vocab_size) 
        
        loss= nn.CrossEntropyLoss(ignore_index= self.target_vocab.char_pad,reduction='sum' )
        output= loss(scores.view( -1,self.vocab_size ) ,char_sequence[1:,:].view(-1) )
        return  output
        ### END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        #(max_length,batch_size) initialized by  self.target_vocab.start_of_word
        input =  torch.full( (max_length,batch_size),self.target_vocab.start_of_word,device=device ,dtype=torch.long)
        scores,_ = self.forward( input,initialStates )  #(length, batch_size, self.vocab_size)
        
        #get max char_id  for  (length,batch,vocab_size) dim=2
        current_char_ids =  torch.argmax(scores,dim= 2).transpose(0,1).detach().numpy().tolist()    #(batch,length )  for char_id
        #for each  word(batch)_char_ids remove end\start_of_word char_id and ultimately concate filtered char
        decodeWords= [ ''.join([self.target_vocab.id2char[char_id]  for char_id in l  if (char_id !=self.target_vocab.start_of_word) and (char_id !=self.target_vocab.end_of_word)  ])     
                                for l in  current_char_ids ]
        return decodeWords        
        ### END YOUR CODE
