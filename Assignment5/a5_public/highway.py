...
class Highway(nn.Module):
    ...
    def __init__(self,word_embed_size , dropout_rate=0.5):
        ...
        self.w_proj= nn.Linear(self.embedding_size,self.embedding_size,bias=True)
        self.w_gate=  nn.Linear(self.embedding_size,self.embedding_size,bias=True)
    ...
    def forward(self, x):
        ...
        """
        x_proj = nn.functional.relu(self.w_proj(x))  #(b,e)
        x_gate= torch.sigmoid( self.w_gate(x))  #(b,e)
        x_highway= x_gate* x_proj+(1-x_gate)*x  #(b,e)
