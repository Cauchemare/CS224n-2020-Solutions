...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder=nn.LSTMCell(embed_size+self.hidden_size ,self.hidden_size,bias=True)  #(e+h,h)
        ...
        self.att_projection=nn.Linear(2* self.hidden_size, self.hidden_size,bias =False) #修改为  nn.Linear(2* self.hidden_size,self.hidden_size,bias =False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ###         that there is no initial hidden state or cell for the decoder(encoder?).
        ...
        init_decoder_hidden= self.h_projection(   torch.cat( ( last_hidden[0],last_hidden[1]), dim= 1) ) #(2,b,h)
        init_decoder_cell = self.c_projection(   torch.cat( ( last_cell[0],last_cell[1]), dim= 1) )  #(2,b,h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # (b, src_len, h) @（b,h,1) ->  （b,src_len, 1) ->(b,src_len)
